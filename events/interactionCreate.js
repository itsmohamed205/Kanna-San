...
client.on("interactionCreate", async (interaction) => {
    ...
    if (interaction.isCommand()) {
        ...
        if (!cmd)
            return interaction.followUp({ content: "Guess this command got deleted..." });
  ...
  let permsname;
    if(command.permissions === "" || !command.permissions) { permsname = "no name"} else { permsname = command.permissions.toLowerCase().replace("_", " ")}
  ...
  if(!interaction.member.permissions.has(command.permissions || []))
      return interaction.editReply({ embeds: [perms] });
